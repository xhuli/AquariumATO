...
Both former "open questions" are resolved: **Low/Reservoir sensors** are build-flag-controlled (not commented out); **`ArduinoLog`** was removed entirely, its purpose now served by the `TRACE` console. No other open questions are currently tracked — add new ones here with enough context to act on later, and remove them once resolved rather than leaving stale entries.
